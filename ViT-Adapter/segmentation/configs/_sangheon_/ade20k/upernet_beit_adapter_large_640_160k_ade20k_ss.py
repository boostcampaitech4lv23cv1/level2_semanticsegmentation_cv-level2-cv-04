...
_base_ = [
    ...
    '../_base_/datasets/trash_custom.py',
...
model = dict(
    ...
    init_cfg=dict(
        type='Pretrained',
        checkpoint='https://github.com/czczup/ViT-Adapter/releases/download/v0.2.1/upernet_beit_adapter_large_640_160k_ade20k.pth.tar'
    )    
)
img_norm_cfg = dict(
...
train_pipeline = [
    ...
    dict(type='LoadAnnotations'), # 이것도 의심스러운데?
    # dict(type='Resize', img_scale=(640, 640), ratio_range=(0.5, 2.0)), # ratio_range 도 조금 의심스러워
    dict(type='Resize', img_scale=(640, 640)),
    # dict(type='RandomCrop', crop_size=crop_size, cat_max_ratio=0.75), # 이것도
    dict(type='RandomFlip', prob=0.5),
    # dict(type='PhotoMetricDistortion'), # ? 이건 왜
